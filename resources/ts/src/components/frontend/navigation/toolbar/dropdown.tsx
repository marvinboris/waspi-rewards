...
const MenuItem = ({ href, label }: MenuItemProps) => {
    ...
    return <div>
        <Link to={href} className={classNames('block truncate py-[7px] px-4 rounded-lg', location.pathname === href ? 'text-white bg-primary font-bold' : '')}>
            {label}
        </Link>
    </div>
}
...
export default function Dropdown({ children, basePath, list }: DropdownProps) {
    const location = useLocation()
    const active = location.pathname.includes(basePath)
