...
export default function View({ action, empty, children, block, title }: ViewProps) {
    let [isOpen, setIsOpen] = useState<boolean>(false)

    return <>
        <div className={block ? 'flex flex-col h-100 flex-1' : 'inline-block'} onClick={() => setIsOpen(true)}>
            {action}
        </div>
